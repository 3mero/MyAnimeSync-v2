...
import { getMediaByAniListId, getMangaCharacters, getMangaStaff, getMangaReviews, getMangaRecommendations } from '@/lib/anilist';
...
import { Languages, Undo2, Loader2, Info, Star, BookOpen, Layers, User, GitMerge, Bookmark, Users, UserCog, MessageSquare, Heart, BookUser, Check, ListVideo, Trash2, CheckCheck, PlusCircle, ArrowUp, ArrowDown, Bell, Search } from 'lucide-react';
    ...
    <Card>
        <CardHeader>
            <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-3">
                    <Image src={review.user.images.webp.image_url} alt={review.user.username} width={40} height={40} className="rounded-full" />
                    <div>
                        <a href={review.user.url} target="_blank" rel="noopener noreferrer" className="font-bold hover:underline">{review.user.username}</a>
                 ...
                 <div className={cn("text-right space-y-2", lang === 'ar' ? 'text-left' : 'text-right')}>
                    <Badge variant={review.score > 6 ? "default" : review.score > 3 ? "secondary" : "destructive"} className="flex items-center gap-1">
        ...
        <CardContent>
            <div className="relative">
                <p className={cn("text-sm text-foreground/80 whitespace-pre-wrap leading-relaxed", !isExpanded && "line-clamp-6", lang === 'ar' && showTranslation && 'rtl')}>
                    {reviewText}
                </p>
...
function ChaptersTab({ manga, listData }: { manga: Anime; listData: any }) {
    ...
    const generateEpisodesFromCustomLink = (anime: Anime, linkInfo: { template: string }, listData: any): Partial<any>[] => {
    ...
    const handleItemClick = useCallback((chapterId: string) => {
                     ...
                     <ScrollArea className="h-96">
                        <div className="space-y-2 pr-4">
                            {sortedChapters.map((ep, index) => {
                                ...
                                const chapterNumber = chapterNumberMatch ? parseInt(chapterNumberMatch[0], 10) : index + 1;
                                    ...
                                    >
                                        ...
                                        {authMode !== 'none' && (
                                            <Button variant="ghost" size="icon" className="shrink-0"
                                                onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleItemClick(chapterId); }}
                                            >
...
export default function MangaPage() {
    ...
    const { 
        authMode, 
        listData,
        isPlannedToRead, togglePlanToRead,
        isCurrentlyReading, toggleCurrentlyReading
    } = useAuth();
    const { toast } = useToast();
    ...
    const handleTabChange = useCallback(async (tab: string) => {
        ...
        // Do not fetch if data exists or it's not a fetchable tab
        if (key === 'about' || key === 'overview' || key === 'chapters' || additionalData[key] !== null) return;
  
        try {
          let data;
          switch(key) {
              case 'staff': data = await getMangaStaff(anilistId); break;
              case 'reviews': data = await getMangaReviews(anilistId); break;
              case 'recommendations': data = await getMangaRecommendations(anilistId); break;
          }
                        ...
                        <CardContent className="p-4 space-y-3">
                                ...
                                {authMode !== 'none' && (
                                    <>
                                        ...
                                        <ReminderDialog anime={media}>
                                            <Button variant="outline" className="w-full flex-wrap">
                            ...
                            {media.score && (
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-sm text-muted-foreground">({media.scored_by?.toLocaleString()} {t('votes')})</span>
                                </div>
                            )}
                    ...
                    <div>
                     ...
                     <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
                        <TabsList className="grid w-full grid-cols-6">
                            ...
                            <TabsTrigger value="staff"><UserCog className="w-4 h-4 md:mr-2" /><span className="hidden md:inline">{t('staff')}</span></TabsTrigger>
                            <TabsTrigger value="reviews"><MessageSquare className="w-4 h-4 md:mr-2" /><span className="hidden md:inline">{t('reviews')}</span></TabsTrigger>
                                ...
                                <CardContent className="grid grid-cols-2 gap-4">
                                    ...
                                    <div className="space-y-1">
                                     ...
                                     <div className="space-y-1">
                                        ...
                                        <p className="font-bold text-lg flex items-center gap-1"><Heart className="w-4 h-4 text-red-500" /> {media.scored_by?.toLocaleString() || n_a}</p>
                                    </div>
                                    <div className="space-y-1">
                        ...
                        <TabsContent value="recommendations" className="py-4">
                            <RecommendedAnimeList title={t('manga_similar')} animes={additionalData.recommendations || []} animeId={media.id} currentTrail={[{id: media.id, title: media.title}]} />
                        </TabsContent>
                     </Tabs>
                </div>
            </div>
